...
public interface OrderDao {
	...
	@Select("select o_id,scheduleid,o_number,o_totalprice,o_ordertime,uid,o_ordernumber,o_paynumber,flag from `order` where flag=0")
	public List<Order> queryAll();
	
	@Select("select o_id,scheduleid,o_number,o_totalprice,o_ordertime,uid,o_ordernumber,o_paynumber,flag from `order` where uid=#{uid} and flag=0")
